...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for organization in organizations:
            ...
            date_string = "2024-04-04 23:15:05"
            ...
            shifts = Shift.objects.filter(
                ...
                & Q(uuid="751153ba-0548-49e4-ae4b-ed9001324a43")
            )
            ...
            for shift in shifts:
                ...
                # Deleting Attendance objects that are already created.
                shifts_uuids = shifts.values("uuid")
                logging.info(f"shifts.values uuid: {shifts_uuids}")
                logging.info(f"organization: {organization}")
                logging.info(f"date: {attendance_date}")

                attendance_objects_need_to_delete = Attendance.objects.filter(
                    shift__uuid__in=shifts.values("uuid"),
                    organization=organization,
                    date=attendance_date
                )

                logging.info(f"attendance_objects_need_to_delete: {attendance_objects_need_to_delete}")
                logging.info(f"=====================Started updating scans data=====================")

                for attendance_objects_need_to_del in attendance_objects_need_to_delete:
                    scans_of_attendance = attendance_objects_need_to_del.scans.all()
                    logging.info(f"scans_of_attendance: {scans_of_attendance}")
                    logging.info(f"scans_of_attendance count: {scans_of_attendance.count()}")
                    updated_scans_data_count = scans_of_attendance.update(is_computed=False, status="pending")
                    logging.info(f"updated_scans_data count: {updated_scans_data_count}")

                logging.info(f"=====================Ended updating scans data=====================")

                attendance_objects_need_to_delete.delete()
